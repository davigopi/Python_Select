...
class Conexao:
    ...
    def log_site(self, info_site, path_site):
        ...
        if modal in path_site and btn_modal in path_site:
            self.funct.path_modal = path_site[modal]
            self.funct.path_modal_btn = path_site[btn_modal]
    ...
    def percussion_cont_newcon(self):
        # self.funct.driver = self.driver
        # self.funct.path_all_log = self.path_all_log
        self.func_click(path_newcon[contemplacao])  # contemplacao
        self.func_click(path_newcon[contemplacao_2] )  # contemplacao
        self.func_click(path_newcon[resultado_de_assembleia])  # Resultado de Assembleia
        self.func_key(path_newcon[input_grupo], self.grupo_newcon)  # campo grupo
        self.func_click(path_newcon[btn_confirmar])  # botao confirmar


    def get_info(self, table_newcon):
        
        if table_newcon != info:
            return []
        disc_info = {}
        for key in list_path_info:
            disc_info[key] = self.func_get_select(path_newcon[key], tag_text)
        return [disc_info]

    def get_sorteio(self):
        renomear.inf = self.func_get_select(path_newcon[ocorrencia], tag_text)
        inf_ocorrencia = renomear.get_num_string()
        renomear.inf = self.func_get_select(path_newcon[dt_ex_assembleia], tag_text)
        inf_dt_ex_assembleia = renomear.get_date_string()
        if inf_ocorrencia not in self.dict_sorteio:
            self.dict_sorteio[inf_ocorrencia] = [inf_dt_ex_assembleia]
        elif inf_dt_ex_assembleia not in self.dict_sorteio[inf_ocorrencia]:
            self.dict_sorteio[inf_ocorrencia].append(inf_dt_ex_assembleia)

    def get_newcon(self, table_newcon):
        list_dict_info = self.get_info(table_newcon)
        if list_dict_info:
            return list_dict_info
        list_dict_join = []
        quant_meses = mesQuantidade if table_newcon != apur else 1
        for _ in range(quant_meses):
            if btns_df_newcon[table_newcon]:
                if not self.func_click(path_newcon[btns_df_newcon[table_newcon]]):
                    break
            for _ in range(10):  # se não tabela diferente retorna 10 tentativa 
                convert.table = self.func_get_select(path_newcon[df_newcon[table_newcon]], tag_table)
                list_dict_new = convert.table_list_in_disc()
                if list_dict_new in list_dict_join: # se tab não alterou ele já esta na tabela
                    time.sleep(0.5)
                    continue
                break
            else:
                raise BlockExecution("A tabela anterior é a mesma da posterior.")
            list_dict_join += list_dict_new
            self.get_sorteio() if table_newcon == conf else None
            self.func_click(path_newcon[btn_retorna_uma_assembleia])  # retorna uma asembleia
        return list_dict_join
    ...
    #         convert.table = self.func_get_select(path_newcon[df_confirmadas_2], tag_table)
    #         disc_all += convert.table_list_in_disc()
    #         self.func_click(path_newcon[btn_retorna_uma_assembleia])
    #     return disc_all
    ...
    #     disc_all = []
    #     for _ in range(mesQuantidade):
    #         self.func_click(path_newcon[btn_confirmadas_canceladas])    
    #         convert.table = self.func_get_select(path_newcon[df_confirmadas_canceladas], tag_table)
    #         disc_all += convert.table_list_in_disc()
    #         self.func_click(path_newcon[btn_retorna_uma_assembleia])
    #     return disc_all
    ...
    #     disc_all = []
    #     for _ in range(mesQuantidade):
    #         self.func_click(path_newcon[btn_desclassificadas])
    #         convert.table = self.func_get_select(path_newcon[df_desclassificadas], tag_table)
    #         disc_all += convert.table_list_in_disc()
    #         self.func_click(path_newcon[btn_retorna_uma_assembleia])
    #     return disc_all
    ...
    #     disc_all = []
    #     for _ in range(mesQuantidade):
    #         if not self.func_click(path_newcon[btn_sequencia_de_apuracao]):
    #             break
    ...
    #     # convert.table = self.func_get_select(path_newcon[df_confirmadas], tag_table, '')
...
class Antiga:
    ...
    def navegar_newcon_desc(self):
        ...
        funct.path_all_log = self.path_all_log
        funct.tempo = 3
        funct.urls = [btn_confirmadas_canceladas]  # desc aba
        funct.funct()
        print('2')
        # botao desc
